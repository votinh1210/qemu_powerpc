...
GEN_HANDLER(lvebx,0x1F,0x7,0x00,0x00000000,PPC_INTEGER)               //mari
{
       int rs,ra,rb;
       rs = rS(ctx->opcode);
       ra = rA(ctx->opcode);
       ...
       op_ldst(lbz);
          ...
          gen_op_load_gpr_T0(rb);
          gen_op_load_gpr_T1(ra);
          gen_op_add();
          op_ldst(lbz);
       }
         gen_op_store_vec_b(rs);
}

GEN_HANDLER(lvehx,0x1F,0x7,0x1,0x00000000,PPC_INTEGER)          //mari
{
        int rs,ra,rb;
       rs = rS(ctx->opcode);
       ra = rA(ctx->opcode);
 ...
 //      gen_op_store_T1_gpr(rb);
  ...
  //       gen_op_store_T1_gpr(rb);
       ...
       gen_op_store_vec_h(rs);
}   
GEN_HANDLER(lvewx,0x1F,0x7,0x2,0x00000000,PPC_INTEGER)          //mari
{
        int rs,ra,rb;
       rs = rS(ctx->opcode);
       ra = rA(ctx->opcode);
 ...
 //         gen_op_store_T1_gpr(rb);
       ...
       gen_op_store_vec_w(rs);
}   
GEN_HANDLER(lvxlt,0x1F,0x7,0xb,0x00000000,PPC_INTEGER)          //mari
{
       int rs,ra,rb;
       rs = rS(ctx->opcode);
       ra = rA(ctx->opcode);
  ...
  //        gen_op_store_T1_gpr(rb);
       ...
       gen_op_store_vec_w(rs);           //  xl = 4w , for(4)
       gen_op_addi(4);
       op_ldst(lwz);
       gen_op_store_vec_w(rs); 
       gen_op_addi(4);
...
GEN_HANDLER(lvsl,0x1F,0x6,0x00,0x00000000,PPC_INTEGER)      //mari
{
       int rs,ra,rb;
       rs = rS(ctx->opcode);
       ra = rA(ctx->opcode);
       ...
       gen_op_init_vec_l(rs);    
}
GEN_HANDLER(lvsr,0x1F,0x6,0x01,0x00000000,PPC_INTEGER)      //mari
{
       int rs,ra,rb;
       rs = rS(ctx->opcode);
       ra = rA(ctx->opcode);
       ...
       gen_op_init_vec_r(rs);    
}
GEN_HANDLER(stvebx,0x1F,0x7,0x04,0x00000000,PPC_INTEGER)     //mari
{
       int rs,ra,rb;
       rs = rS(ctx->opcode);
       ra = rA(ctx->opcode);
       ...
       gen_op_load_gpr_T0(rb);  
         ...
         gen_op_load_vec_b(rs);
         op_ldst(stb);
         gen_op_printf_mem_s();
       
}
GEN_HANDLER(stvehx,0x1F,0x7,0x05,0x00000000,PPC_INTEGER)     //mari
{
       int rs,ra,rb;
       rs = rS(ctx->opcode);
       ra = rA(ctx->opcode);
       ...
       gen_op_load_gpr_T0(rb);  
         ...
         gen_op_load_vec_h(rs);
         op_ldst(sth);
         gen_op_printf_mem_s();
       
}
GEN_HANDLER(stvewx,0x1F,0x7,0x06,0x00000000,PPC_INTEGER)     //mari
{
       int rs,ra,rb;
       rs = rS(ctx->opcode);
       ra = rA(ctx->opcode);
       ...
       gen_op_load_gpr_T0(rb);  
         ...
         gen_op_load_vec_w(rs);
         op_ldst(stw);
         gen_op_printf_mem_s();
       
}
GEN_HANDLER(stvx1,0x1F,0x7,0x07,0x00000000,PPC_INTEGER)     //mari
{
       int rs,ra,rb;
       rs = rS(ctx->opcode);
       ra = rA(ctx->opcode);
                                                         ...
                                                         int search_pc)
{
    ...
    while (ctx.exception == POWERPC_EXCP_NONE && gen_opc_ptr < gen_opc_end) {
        ...
        if (unlikely(little_endian)) {
          ...
          #ifdef GDB_ENABLED
             gen_op_gdb_verify(ctx.nip);
          #endif
       //   printf("r1 = %0x\n",env->gpr[1]);
       // printf("op = %0x\n",ctx.opcode);                       //mari
       // printf("nip = %0x op = %0x\n",ctx.nip,ctx.opcode); 
      // printf("translate opcode %08x (%02x %02x %02x)\n",ctx.opcode, opc1(ctx.opcode), opc2(ctx.opcode),opc3(ctx.opcode));
        ctx.nip += 4;
        table = env->opcodes;
