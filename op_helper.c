...
#include "op_helper_mem.h"
#endif
int mask_m_b[4]={0xffffff00,0xffff00ff,0xff00ffff,0x00ffffff};
int mask_m_h[4]={0xffff0000,0x0000ffff};
int mask_m_w = 0xffffffff;
...
void do_printf_sun(int c,int j,int i)        //mari
{
    
    printf("%x %x %x\n",c,j,i);  
}  
void do_print_xx(int i,int j,int k)
{    
    printf("aaaaa%d %d %d\n",i,j,k);  
}
void do_store_vector_b(int addr, int data ,int i)                    //mari
{
    int t,t1;
   addr = addr & 0xf;
    if(addr<4)
    {
         t1 = 3 - addr; 
         env->vector_gpr[i].low =  env->vector_gpr[i].low & mask_m_b[t1];
         env->vector_gpr[i].low =  env->vector_gpr[i].low | (data<<(t1*8));
     }
     else if((addr>=4)&&(addr<8))
     {
          t = addr%4;
          t1 = 3 - t;
          env->vector_gpr[i].low_h =  env->vector_gpr[i].low_h & mask_m_b[t1];
          env->vector_gpr[i].low_h =  env->vector_gpr[i].low_h | (data<<(t1*8));    
      }
     else if((addr>=8)&&(addr<12))
     {
          t = addr%4;
          t1 = 3 - t;
          env->vector_gpr[i].high_l =  env->vector_gpr[i].high_l & mask_m_b[t1];
          env->vector_gpr[i].high_l =  env->vector_gpr[i].high_l | (data<<(t1*8));
      }
      else if((addr>=12)&&(addr<16))
     {
          t = addr%4;
          t1 = 3 - t;
          env->vector_gpr[i].high =  env->vector_gpr[i].high & mask_m_b[t1];
          env->vector_gpr[i].high =  env->vector_gpr[i].high | (data<<(t1*8));   
      }

}
void do_store_vector_h(int addr, int data ,int i)                    //mari
{
    int t,t1;
    addr = addr & 0xf;
    if(addr<4)
    {
         t1 = 2 - addr;
         env->vector_gpr[i].low =  env->vector_gpr[i].low & mask_m_h[t1];
         env->vector_gpr[i].low =  env->vector_gpr[i].low | (data<<(t1*8));
     }
     else if((addr>=4)&&(addr<8))
     {
          t = addr%4;
          t1 = 2 - t;
          env->vector_gpr[i].low_h =  env->vector_gpr[i].low_h & mask_m_h[t1];
          env->vector_gpr[i].low_h =  env->vector_gpr[i].low_h | (data<<(t1*8));    
      }
     else if((addr>=8)&&(addr<12))
     {
          t = addr%4;
          t1 = 2 - t;
          env->vector_gpr[i].high_l =  env->vector_gpr[i].high_l & mask_m_h[t1];
          env->vector_gpr[i].high_l =  env->vector_gpr[i].high_l | (data<<(t1*8));
      }
      else if((addr>=12)&&(addr<16))
     {
          t = addr%4;
          t1 = 2 - t;
          env->vector_gpr[i].high =  env->vector_gpr[i].high & mask_m_h[t1];
          env->vector_gpr[i].high =  env->vector_gpr[i].high | (data<<(t1*8));   
      }
}
void do_store_vector_w(int addr, int data ,int i)                    //mari
{
    addr = addr & 0xf;
    if(addr<4)
    {
         env->vector_gpr[i].low =  env->vector_gpr[i].low & mask_m_w;
         env->vector_gpr[i].low =  env->vector_gpr[i].low | data;
     }
     else if((addr>=4)&&(addr<8))
     {
          env->vector_gpr[i].low_h =  env->vector_gpr[i].low_h & mask_m_w;
          env->vector_gpr[i].low_h =  env->vector_gpr[i].low_h | data;    
      }
     else if((addr>=8)&&(addr<12))
     {
          env->vector_gpr[i].high_l =  env->vector_gpr[i].high_l & mask_m_w;
          env->vector_gpr[i].high_l =  env->vector_gpr[i].high_l | data;
      }
      else if((addr>=12)&&(addr<16))
     {
          env->vector_gpr[i].high =  env->vector_gpr[i].high & mask_m_w;
          env->vector_gpr[i].high =  env->vector_gpr[i].high | data;   
      }
}
int do_load_vec_b(int addr , int i)      //mari
{ 
    int t,t1,data = 0;
    addr = addr & 0xf;
    if(addr<4)
    {
         t1 = 3-addr; 
         data =  env->vector_gpr[i].low & (~mask_m_b[t1]);
         data = data>>(t1*8);
     }
     else if((addr>=4)&&(addr<8))
     {
          t = addr%4;
          t1 = 3 - t;
          data =  env->vector_gpr[i].low_h & (~mask_m_b[t1]);
          data = data>>(t1*8);
      }
     else if((addr>=8)&&(addr<12))
     {
          t = addr%4;
          t1 = 3 - t;
           data =  env->vector_gpr[i].high_l & (~mask_m_b[t1]);
         data = data>>(t1*8);
      }
      else if((addr>=12)&&(addr<16))
     {
          t = addr%4;
          t1 = 3 - t;
          data =  env->vector_gpr[i].high & (~mask_m_b[t1]);
          data = data>>(t1*8);
      }
     return data;
}
int do_load_vec_h(int addr , int i)      //mari
{ 
    int t,t1,data = 0;
    addr = addr & 0xf;
    if(addr<4)
    {
         t1 = 2-addr; 
         data =  env->vector_gpr[i].low & (~mask_m_h[t1]);
         data = data>>(t1*8);
     }
     else if((addr>=4)&&(addr<8))
     {
          t = addr%4;
          t1 = 2 - t;
          data =  env->vector_gpr[i].low_h & (~mask_m_h[t1]);
          data = data>>(t1*8);
      }
     else if((addr>=8)&&(addr<12))
     {
          t = addr%4;
          t1 = 2 - t;
           data =  env->vector_gpr[i].high_l & (~mask_m_h[t1]);
         data = data>>(t1*8);
      }
      else if((addr>=12)&&(addr<16))
     {
          t = addr%4;
          t1 = 2 - t;
          data =  env->vector_gpr[i].high & (~mask_m_h[t1]);
          data = data>>(t1*8);
      }
     return data;
}
int do_load_vec_w(int addr , int i)      //mari
{ 
    int data = 0;
    addr = addr & 0xf;
    if(addr<4)
    {
         data =  env->vector_gpr[i].low;
     }
     else if((addr>=4)&&(addr<8))
     {
          data =  env->vector_gpr[i].low_h;
      }
     else if((addr>=8)&&(addr<12))
     {
         data =  env->vector_gpr[i].high_l;
      }
      else if((addr>=12)&&(addr<16))
     {
          data =  env->vector_gpr[i].high;
      }
     return data;
}
void do_printf_mem_s(int addr)
{
    int data,addr1;
    addr1 = addr % 4;
    addr = addr - addr1;
...
void do_init_vec_l(int addr ,int j)        //mari
{
    addr = addr & 0xf;
    switch(addr){
      case 0:
          env->vector_gpr[j].low = 0x00010203;
          env->vector_gpr[j].low_h = 0x04050607;
          env->vector_gpr[j].high_l = 0x08090a0b;
          env->vector_gpr[j].high = 0x0c0d0e0f;
          break;
      case 1:
          env->vector_gpr[j].low = 0x01020304;
          env->vector_gpr[j].low_h = 0x05060708;
          env->vector_gpr[j].high_l = 0x090a0b0c;
          env->vector_gpr[j].high = 0x0d0e0f10;
          break;
      case 2:
          env->vector_gpr[j].low = 0x02030405;
          env->vector_gpr[j].low_h = 0x06070809;
          env->vector_gpr[j].high_l = 0x0a0b0c0d;
          env->vector_gpr[j].high = 0x0e0f1011;
          break;
      case 3:
          env->vector_gpr[j].low = 0X03040506;
          env->vector_gpr[j].low_h = 0x0708090a;
          env->vector_gpr[j].high_l = 0x0b0c0d0e;
          env->vector_gpr[j].high = 0x0f101112;
          break;
     case 4:
          env->vector_gpr[j].low = 0x04050607;
          env->vector_gpr[j].low_h = 0x08090a0b;
          env->vector_gpr[j].high_l = 0x0a0d0e0f;
          env->vector_gpr[j].high = 0x10111213;
          break;
     case 5:
          env->vector_gpr[j].low = 0x05060708;
          env->vector_gpr[j].low_h = 0x090a0b0c;
          env->vector_gpr[j].high_l = 0x0d0e0f10;
          env->vector_gpr[j].high = 0x11121314;
          break;
     case 6:
          env->vector_gpr[j].low = 0x06070809;
          env->vector_gpr[j].low_h = 0x0a0b0c0d;
          env->vector_gpr[j].high_l = 0x0e0f1011;
          env->vector_gpr[j].high = 0x12131415;
          break;
     case 7:
          env->vector_gpr[j].low = 0x0708090a;
          env->vector_gpr[j].low_h = 0x0b0c0d0e;
          env->vector_gpr[j].high_l = 0x0f101112;
          env->vector_gpr[j].high = 0x13141516;
          break;
     case 8:
          env->vector_gpr[j].low = 0x08090a0b;
          env->vector_gpr[j].low_h = 0x0c0d0e0f;
          env->vector_gpr[j].high_l = 0x10111213;
          env->vector_gpr[j].high = 0x14151617;
          break;
     case 9:
          env->vector_gpr[j].low = 0x090a0b0c;
          env->vector_gpr[j].low_h = 0x0d0e0f10;
          env->vector_gpr[j].high_l = 0x11121314;
          env->vector_gpr[j].high = 0x15161718;
          break;
     case 10:
          env->vector_gpr[j].low = 0x0a0b0c0d;
          env->vector_gpr[j].low_h = 0x0e0f1011;
          env->vector_gpr[j].high_l = 0x12131415;
          env->vector_gpr[j].high = 0x16171819;
          break;
     case 11:
          env->vector_gpr[j].low = 0x0b0c0d0e;
          env->vector_gpr[j].low_h = 0x0f101112;
          env->vector_gpr[j].high_l = 0x13141516;
          env->vector_gpr[j].high = 0x1718191a;
          break;
     case 12:
          env->vector_gpr[j].low = 0x0c0d0e0f;
          env->vector_gpr[j].low_h = 0x10111213;
          env->vector_gpr[j].high_l = 0x14151617;
          env->vector_gpr[j].high = 0x18191a1b;
          break;
    case 13:
          env->vector_gpr[j].low = 0x0d0e0f10;
          env->vector_gpr[j].low_h = 0x11121314;
          env->vector_gpr[j].high_l = 0x15161718;
          env->vector_gpr[j].high = 0x191a1b1c;
          break;
    case 14:
          env->vector_gpr[j].low = 0x0e0f1011;
          env->vector_gpr[j].low_h = 0x12131415;
          env->vector_gpr[j].high_l = 0x16171819;
          env->vector_gpr[j].high = 0x1a1b1c1d;
          break;
    case 15:
          env->vector_gpr[j].low = 0x0f101112;
          env->vector_gpr[j].low_h = 0x13141516;
          env->vector_gpr[j].high_l = 0x1718191a;
          env->vector_gpr[j].high = 0x1b1c1d1e;
          break;
    default:
          break;        
    }
...
void do_init_vec_r(int addr ,int j)        //mari
{
    addr = addr & 0xf;
    switch(addr){
      case 0:
          env->vector_gpr[j].low = 0x10111213;
          env->vector_gpr[j].low_h = 0x14151617;
          env->vector_gpr[j].high_l = 0x18191a1b;
          env->vector_gpr[j].high = 0x1c1d1e1f;
          break;
      case 1:
          env->vector_gpr[j].low = 0x0f101112;
          env->vector_gpr[j].low_h = 0x13141516;
     ...
     case 12:
          env->vector_gpr[j].low = 0x04050607;
          env->vector_gpr[j].low_h = 0x08090a0b;
          env->vector_gpr[j].high_l = 0x0c0d0e0f;
          env->vector_gpr[j].high = 0x10111213;
          break;
    case 13:
          env->vector_gpr[j].low = 0x03040506;
    ...
    case 14:
          ...
          env->vector_gpr[j].low_h = 0x06070809;
          env->vector_gpr[j].high_l = 0x0a0b0c0d;
          env->vector_gpr[j].high = 0x0e0f01011;
          break;
    case 15:
          env->vector_gpr[j].low = 0x01020304;
          env->vector_gpr[j].low_h = 0x05060708;
...
void do_printf_vector(int i)            //mari
{
     printf("vector gpr[%d],%x %x %x %x\n",i,env->vector_gpr[i].low,env->vector_gpr[i].low_h,env->vector_gpr[i].high_l,env->vector_gpr[i].high);   
}
void do_store_cr (uint32_t mask)          //mari
{
    int i, sh;
...
void do_load_dcr (void)
{
    ...
    if (unlikely(!(env->dcr_env == NULL))) {                                    //mari  dcr_env no define  using other method to achieve the dcr         
        if (loglevel != 0) {
    ...
    else if (unlikely(ppc_dcr_read(env->dcr_env, T0, &val) != 0)) {                        //$$mari
        if (loglevel != 0) {
